function [C,CS,Call,C0]=time_frequency_wavelet(x,fw,fs,pr,opt,mode,varargin)
...
% mode:
% MATLAB or CPU runs the regular matlab cwt
% CPUtest runs cwt on the CPU but uses fft for convolution. Runs usually
% faste than CPU/MATLAB
% mGPU runs cwt on the GPU and uses fft for convolution - no significant
% speed up vs. CPUtest
%
% GPU old outdated GPU mode, requires Jacket pulgin
%
% fftCPU uses cwtft instead of cwt. Runs 2 times faster than CPUtest
% fftGPU uses cwtft on the GPU, 5x speedup vs. CPUtest - both methods
% estimate phase poorly
...
if(strcmp(mode,'fftGPU'))
    ...
    scales=fs./fw;
    xg=gpuArray(x);
    ergf=gather(cwtft(xg,'wavelet',{wvlt,s0},'scales',scales));
    Call=squeeze(permute(reshape(ergf.cfs, [length(fw),size(x,1),size(x,2)]),[2 1 3]));
    C=mean(abs(Call),3);
end
if(strcmp(mode,'fftCPU'))
     C0=[];
    CS=[];
     wvlt='morlex';
    scales=fs./fw;
    ergf=cwtft(x,'wavelet',wvlt,'scales',scales);
    Call=squeeze(permute(reshape(ergf.cfs, [length(fw),size(x,1),size(x,2)]),[2 1 3]));
    C=mean(abs(Call),3);
end
if(strcmp(mode,'GPUold'))
    ...
    CS=[];
    scales=fs./fw*2;
    fall=cwt_felix(x,scales,wvlt,mode);
...
if(strcmp(mode,'mGPU'))
    ...
    % move data and parameters to GPU
    wav=gpuArray(fall(3:end,:));
    wav_length=gpuArray(fall(1,:)');
    scal=gpuArray(fall(2,:)');
    ix1g=gpuArray(ix1);
    ix2g=gpuArray(ix2);
    xg=gpuArray(complex(x));
    gopt=gpuArray(opt);
    n=size(xg,2);
    nsc=size(wav,2);
    Cg=complex(gpuArray(zeros(size(xg,1)+size(wav,1)-2,nsc)));
    if(gopt)
        Cgall=gpuArray(complex(zeros(size(xg,1),n,nsc)));
    end
    for k=1:nsc
    ...
    if(gopt)
        Call=permute(gather(Cgall),[1 3 2]);
        C=mean(abs(Call),3);
    else
        Call=[];
        C=abs(gather(Cg));
...
if(strcmp(mode,'CPUtest'))
    scales=fs./fw*2;
    fall=cwt_felix(x,scales,wvlt,'GPU');
    ix1=zeros(size(fall,2),1);
    ix2=ix1;
    for k=1:size(fall,2)
        la=fall(1,k);
